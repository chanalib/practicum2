...
const MusicAnimation = () => {
  ...
  useEffect(() => {
    ...
    if (!canvas) return // בדיקה אם canvas הוא null

    const ctx = canvas.getContext("2d")
    if (!ctx) return // בדיקה אם ctx הוא null
    ...
    class MusicParticle {
      ...
      update() {
        ...
        if (this.y < 0) {
          this.y = this.canvasHeight // השתמש בגובה הקנבס
          this.x = Math.random() * this.canvasWidth // השתמש ברוחב הקנבס
        }
    ...
    for (let i = 0; i < particleCount; i++) {
      particles.push(new MusicParticle(canvas.width, canvas.height)); // העברת רוחב וגובה
    }
